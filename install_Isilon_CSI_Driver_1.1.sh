...
#Author: Anjan Dave
#Version: 1.0
printf "\n\tThis interactive script installs the CSI Driver 1.0. Try to run it once only as there's not a ton of logic built-in to check for already installed components if you re-run the script.\n"
...
printf "\nNOTE: Kubernetes 1.14 is expected, if you used my other scripts in the repo to install docker & kubernetes, you should be fine \n\n"
sleep 10 
...
then
	...
	git clone https://github.com/acd13/isilon-csi-script.git
...
then
	...
	if [ $? -eq 0 ]
 	then
		...
		printf "\nDownloading and running tiller component of helm now...\n"
		sleep 10
		cd /root
		#curl https://raw.githubusercontent.com/helm/helm/master/scripts/get > get_helm.sh
		curl -L https://git.io/get_helm.sh | bash -s -- --version v2.15.2
